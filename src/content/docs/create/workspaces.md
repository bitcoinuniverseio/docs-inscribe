---
<<<<<<<< HEAD:src/content/docs/create/workspaces.md
title: 'Workspace map'
...
========
title: Workspace map
description: Every workspace in Inscribe, what it is for, the four fastest ways to reach one, and what the availability labels mean.
provenance:
  ...
  lastVerified: 2026-09-01
---
>>>>>>>> f81b73edc3d97beddafed9ab57fbf3530bd423ec:src/content/docs/reference/workspaces.md

Inscribe has one workspace per job. This page lists all of them, in the order the sidebar
shows them, so you can find the right one without opening each.

## Four routes faster than the sidebar

- The **search box** in the header.
- The **Mint menu** beside it, which lists every workspace that creates something, with a
  line saying what each one does.
- The **command palette**, `Ctrl` + `K` on Windows and Linux, `Cmd` + `K` on macOS.
- The **paste box on Home**. Paste an address, a transaction id, an inscription ID or a
  ticker and Inscribe opens the workspace that handles it.

All four read the same list, so a workspace appears in every one of them or in none,
described the same way.

Most workspaces also have a guide under **Docs and Guides** in the app, with the exact field
names, the diagrams, and worked examples. This page is the map; those guides are the detail.
...
| Ord Studio | Full inscription-envelope control: live HTML sandbox, templates, and the ord specification fields (pointer, parent, metadata, gzip, metaprotocol) |
| AVM Studio | Build, test, deploy, call and verify Atomicals AVM contracts, with the academy alongside the editor. **Not released** |
| ChainBloom | Grow a shared world one contribution at a time |
| Witness Circles | Taproot outputs a group authorises together. **Not released** |
| Send Inscriptions | Move inscriptions or outputs you already hold |
...
Most protocol workspaces share one shape: browse a token list, fill a Deploy, Mint or
...
| src-20 | Deploy, mint and transfer SRC-20. Data lives in 330-satoshi P2WSH outputs; your wallet signs pre-built transactions |
...
| op20 | OP-20 token deploy, mint and transfer, plus OP Names registration and transfer |
...
| drop pacts | Design and launch Drop Pact agreements, with a resumable tutorial. **Not released** |
...
| Workspace | What it is for |
| --- | --- |
| Tandem | Build and change one object together, with a signed record of each change. **Not released** |
| Patina | How long a value-carrying output has stayed where it is. **Not released** |
...
| cat-20 | Explore CAT-20 on Fractal, check balances, and prepare covenant mints. You pick the network yourself. **Not released** |
| tap on doge | Mint TAP tokens indexed on Dogecoin, including Doge DMT blocks. Mint only |
| dust-20 | Deploy and mint DUST-20, where the mint output satoshi value encodes the amount |
...
an output before you sign it. See
...
| Workspace | What it is for |
| --- | --- |
| Docs and Guides | 63 in-app guides across getting started, create, protocols, workflows, wallet, intel, safety and troubleshooting |
...
- **Unsupported**: the source is not running for this network. Nothing is guessed or filled
  in from elsewhere.
...
========
[Where the data comes from](/docs-inscribe/concepts/source-freshness/) explains where these
states come from and what each one blocks.

## Two workspaces that return a 404 when switched off

Most gated workspaces load and refuse to act. Two do not, and return the 404 page instead:

- **Recover Sats**, at `/recover-sats`
- **The Dogecoin BlockPad**

A 404 at either is a switched-off feature, not a broken link.
